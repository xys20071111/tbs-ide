...
export class LanguageDetectionSimpleWorker extends EditorSimpleWorker {
	...
	private adjustLanguageConfidence(modelResult: ModelResult): ModelResult {
		switch (modelResult.languageId) {
			...
			// these are commonly used languages in TBS-IDE and supported
			// by the model.
			case 'js':
			case 'html':
			case 'json':
			...
			// languages that are provided by default in TBS-IDE
			case 'bat':
			case 'ini':
			case 'makefile':
			case 'sql':
			// languages that aren't provided by default in TBS-IDE
			case 'csv':
			case 'toml':
